...
from online_restaurant_db import Menu, Orders, Reservation, Session, Users
import secrets
...
TABLE_NUM = {
    "1-2": 5,   # 5 столів для 1-2 людей
    "3-4": 3,   # 3 стола для 3-4 людей
    "4+": 2     # 2 стола для великих компаній (4+)
}
...
@app.route('/test_basket', methods = ['GET','POST'])
def test_basket():
    basket = session.get('basket', {})
...
def reserved():
    if request.method == "POST":
        ...
        # Получаем дату и время из формы и превращаем строку в datetime
        reserved_time_start = datetime.fromisoformat(request.form['time'])
        ...
        with Session() as cursor:
            ...
            reserved_check = (
                ...
                .filter_by(type_table=table_type)
                .count()
            )
            ...
            message = (
                f'Бронь на {reserved_time_start.strftime("%d.%m.%Y о %H:%M")} '
                f'столика на {table_type} людини успішно створено!'
            )
...
def reservations_check():
    ...
    if request.method == "POST":
        ...
        with Session() as cursor:
            ...
            reservation = (
                cursor.query(Reservation)
                .filter_by(id=reserv_id)
                .first()
            )

            # Если бронь не найдена
            if reservation is None:
                return redirect(url_for('reservations_check'))

            # Удаляем найденную бронь
            cursor.delete(reservation)
            cursor.commit()

        # После удаления возвращаемся к списку броней
        return redirect(url_for('reservations_check'))
...
def cancel_order(id):
    ...
    with Session() as cursor:
        order: Orders = cursor.query(Orders).filter_by(id=id).first()
        if not order:
            flash('Такого замовлення немає', 'danger')
            return redirect(url_for('my_orders'))
        
        if order.user_id != current_user.id:
            flash('У вас немає прав на скасування цього замовлення', 'danger')
            return redirect(url_for('my_orders'))
